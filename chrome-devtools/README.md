...
To take only this one, use the object form in `settings.json` and point it at this directory:
...
Or load a local checkout for a single session:

```bash
npm install
pi -e ./chrome-devtools
```
